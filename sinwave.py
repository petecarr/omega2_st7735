...
colors = [VIOLET, INDIGO, BLUE, CYAN, GREEN, YELLOW, ORANGE, RED, MAGENTA, WHITE]
...
def setColor(red, green, blue):
    #color =  "#%02x%02x%02x" % (red&255, green&255, blue&255)
...
def drawSin():
    global x, y
    #bleu = random.randint(0,31)
    #random.randint(0,63)
    #random.randint(0,31)
    #col = setColor(rouge,vert,bleu)
    idx = random.randint(0,len(colors)-1)
    col = colors[idx]
    xcenter = CANVASHEIGHT//2
    ycenter = CANVASWIDTH//4
    rad = ycenter-4
    #print("Circle radius {0:3d}, center ({1:3d},{2:3d})".format(
    #    rad,xcenter, ycenter))
    tft.drawCircle(xcenter, ycenter, rad, col)
    tft.drawLine(CANVASHEIGHT//2, 1, CANVASHEIGHT//2, CANVASWIDTH-1, col)
    #print("line:",CANVASHEIGHT//2, 1, CANVASHEIGHT//2, CANVASWIDTH-1)
    ...
    for i in rev:
        # delete last pointer
        if i != 0:  # actually prettier without this
            pass
            #tft.drawLine(xcenter, ycenter, lastx[i],lasty[i], BLACK)
        # draw new pointer
        tft.drawLine(xcenter, ycenter, x[i],y[i], col)
        # draw sin wave increment
        #print("Sin: {0:3d}, {1:3d},{2:3d},{3:3d}".format(xx,lasty,xx+1,y[i]))
        #tft.drawLine(lastx, yy, x[i], yy+1, col)
        if x[i] > xcenter:
        ...
        lastx = x[i]
        lasty = y[i]
    # put update in the loop for a slower, animated, display
    #tk.update()
