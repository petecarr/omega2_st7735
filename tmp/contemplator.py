...
def drawCircleOfCircles(colorIndex, 
    ...
    cy = WIDTH_LANDSCAPE  / 2
    cx = HEIGHT_LANDSCAPE / 2
...
def concentricSquaresPattern():
    ...
    while ((x >= 0) and (x+w < WIDTH_LANDSCAPE) and
      (y >= 0) and (y+h < HEIGHT_LANDSCAPE)):
...
if __name__ == '__main__':                                           
    ...
    try:                                    
        print("Contemplator")
        print("Enter cntrl-C to get next display, cntrl-Z to quit")      
        setup()                             
        while (True):                       
            try:
                nextDisplay()                          
            except KeyboardInterrupt:               
                print("Keyboard interrupt, next display")
                subprocess.call(["./free_mosi"])
    except KeyboardInterrupt:               
